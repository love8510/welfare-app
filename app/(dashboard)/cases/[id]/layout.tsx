  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-4">
        {/* 탭 사이드바 */}
        <nav className="w-44 flex-shrink-0 bg-white rounded-lg border overflow-hidden">
          <Link
            href={`/cases/${id}`}
            className={cn(
              'block px-4 py-2.5 text-sm border-b transition-colors font-semibold text-gray-700',
              pathname === `/cases/${id}` ? 'bg-green-50 text-[#2d7a2d]' : 'hover:bg-gray-50'
            )}
          >
            사례 개요
          </Link>
        ...
        {/* 콘텐츠 영역 */}
        <div className="flex-1 min-w-0 bg-white rounded-lg border p-6 overflow-auto">
          {children}
        </div>
      </div>
    </div>
